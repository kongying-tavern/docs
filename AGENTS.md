...
Run `corepack enable && pnpm install` before first use. `pnpm dev` launches VitePress locally. `pnpm build` (alias `pnpm build-vitepress`) produces the static docs served at `/docs/`. `pnpm build:fonts` validates or regenerates cached font subsets before each production build: site characters only affect the `min` layer in `fonts-subset.css`, while pinned Google Fonts buckets keep the L1/L2/L3 WOFF2 files and `fonts-standard.css` stable. Font pipeline settings live in `scripts/fonts/font-subset.config.json`; standard characters and Google slicing buckets live in the referenced `font-characters.config.json` and `font-slicing.config.json`. The build requires Python 3 with fonttools + brotli (`pip install -r requirements.txt`) and stops when this toolchain is unavailable. Use `pnpm build-data` when updating member or blog data; the command chains `use-data-env`, `build-member`, `pnpm lint`, `pnpm typecheck`, and `pnpm lint:md` keep TS, Vue, and Markdown quality in check. For Chinese copy edits run `pnpm lint:zh` (and `pnpm lint:zh-fix` to auto-fix common issues).
