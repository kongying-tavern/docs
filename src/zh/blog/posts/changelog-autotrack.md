...
* 下载热更新资源时将会带上唯一sid，重试时将会尝试不同的服务器，提高更新成功率
...
* 使用自行实现的三自由度求解器代替opencv的四自由度求解器，求解的坐标更精确
* 使用新的高效率索引算法HNSW（分层导航小世界）
  * 缓存大小：57.8MB--> 221MB  缓存大小增加，但依然小于旧版
  * 内存占用：455MB --> 357MB  进一步降低内存占用
  * 全局匹配效率极高，接近局部匹配
  * 修复上一个版本LSH索引性能不稳定的问题

:::

::: timeline Beta-6.7.2

* 使用更加轻量的匹配算法TEBLID，大幅降低缓存大小和匹配时内存占用
  * 缓存大小：405MB --> 57.8MB
  * 内存占用：865MB --> 455MB
  * 全局匹配干扰点更少，成功率大幅提升
...
* <del>添加惯性导航功能，在追踪失败时会提供至少一分钟的位置预测，防止位置停滞，且尽可能提高局部匹配的命中率，减少全局匹配产生的CPU占用</del>
* 支持HDR屏幕下的位置追踪，目前已适配 原神原生HDR/RTX HDR/Window Auto HDR
* 修复因为单轮多次截图导致追踪帧率低的问题
* flann索引落盘，进一步减少生成缓存的时间
...
* 适配蒙德·风息山和空之神殿追踪
* 优化特征点检测算法，通过将检测器替换为FAST得到更好的角点检测结果
* 全局匹配模式会尝试所有可能的结果，而不是选择最好的结果（存在假阳性），提高全局匹配模式的成功率，减少瞬移
* 全局匹配模式新增空间索引，匹配效率大幅增加，传送后延迟得到明显改善
* 修复一个遗留问题：底图在6.2.1依然以RGB方式加载，现在修复成正确的表现：灰度加载，降低第一次生成特征点缓存的内存占用
