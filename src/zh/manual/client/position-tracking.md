...
本功能用于在空荧酒馆中同步显示玩家在游戏中的坐标，以方便在酒馆客户端标记玩家位置附近的宝箱。

本功能基于图像识别实现，使用相对安全，不涉及到对游戏内存的读写，不会往游戏中写入叠加层，只会在酒馆客户端同步显示玩家位置，不会影响到游戏的公平性，目前没有封号的案例。

由于基于图像识别，无法保证准确性，会有坐标偏移，坐标抖动，偶发坐标瞬移，未解锁地区无法追踪，大面积遮挡追踪效果下降的问题，这个属于视觉算法的局限，望能理解并不会因为以上问题叨扰开发者。

若想使用本功能，请确认：

- 您的操作版本至少是 **win10 或以上版本** (win7 不支持追踪)
- 由于云原神可能加强了 DRM 保护，有大概率无法抓取到画面，可能无法追踪。
- 使用**官方正版原神客户端**，不限服务器，支持 Unity 客户端。
- 支持非官方启动器，只要游戏本体是官方正版即可。
- 游戏内【解锁神像点亮地图】，游戏内【左上角小地图】完整
- 游戏内【左上角小地图】中，无大面积遮挡 (如黄色的任务范围圆圈)，设置为北向上
- 支持全屏和窗口，推荐使用【无边框全屏】模式运行游戏。
...
部分玩家反馈之前能正常使用位置追踪，有一天突然不能使用，可能是游戏内 UI 界面被遮挡

最常见的情况是「提升指南」提示遮挡左上角 UI 界面
...
目前该问题仅在游戏内 UI 遮挡出现，来自其他软件的悬浮窗遮挡不会影响到追踪使用。

### 关于手柄的说明

由于手柄的 GUI 尺寸略小，只有键鼠尺寸的 85%，其中的信息损失可能会导致追踪质量轻微下降，目前发现在空之神殿的部分地区追踪会比较困难

考虑大部分玩家是键鼠玩家，所以追踪的特征点尺度更接近于键鼠的 GUI 大小

如果发现部分位置虽然特征比较明显，但定位比较困难，可以临时切换为键鼠操作模式

未来打算在 2K 以上分辨率修复这个问题，但如果屏幕尺寸依然是 1080p 则暂时无法解决手柄 Ui 尺寸导致的追踪质量轻微下降的问题

### 窗口黄框说明

在较新的 Windows 版本中，通过 WGC(Windows Graphics Capture)，对应酒馆选项是 DirectX，抓屏会强制在被抓取画面外围标记黄框。

由于酒馆客户端是 Win32 应用，黄框需要宿主应用是 UWP 应用且声明抓屏权限才能去除，所以目前无法去除。

可以尝试使用 bitblt，但目前发现部分系统(尤其是 Win11)使用 bitblt 抓取画面会黑屏，如果不能使用就只能使用 DirectX(WGC)。
...
切换到 bitblt 模式，如果在「位置追踪>追踪截图」获取到的追踪截图有画面，那就可以用，否则无解。

### 关于 HDR 的说明

目前追踪的 WGC(DirectX)已对 HDR 显示器进行适配，目前支持官方 HDR、Windows Auto HDR、RTX HDR

已知限制，HDR 截图需要消耗更多的性能，在较低端的设备可能卡顿，如果遇到这个问题建议在 SDR 下使用

其他显卡的 HDR 功能尚未测试，自测时可以在「设置>位置追踪>追踪截图」获取，然后在「设置>位置追踪>截图目录」中查看颜色是否正确
...
这个目前暂时确定是客户端热更新框架带来的新 bug，会在没有坐标数据的时候卡在这两个位置，由于测试环境无法复现所以暂时没有解决方法。如果长期卡顿请按照上述「位置追踪无反应」处理。

### 瞬移

<del>为了优化匹配速度，传送后会先做全图但比较粗糙的全局匹配，然后使用精细但范围小的局部匹配验证是否正确。但由于匹配结果不可控，会有较小概率在错误的坐标验证通过。</del>

以上问题在 6.5.1 版本之后得到了解决，但出现瞬移依然可以通过以下方式排查。

考虑到误检测到错误位置后，后续局部匹配更容易失败从而重新触发全局匹配，可边让角色行走(让小地图平移从而刷新出新的特征)边等待位置追踪收敛到正确的位置。
