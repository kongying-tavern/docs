import type { CustomConfig } from '../types'

const forum: CustomConfig['forum'] = {
  loadMore: '加载更多',
  noMore: '没有更多',
  loadError: '加载失败',
  lastVisited: '上次浏览到这里',
  empty: {
    title: '暂无反馈',
    description: '可以调整筛选条件，或者提交新的反馈。',
    searchTitle: '搜索“{query}”',
    searchDescription: '没有搜索到任何相关反馈内容。',
  },
  reaction: {
    label: '反馈评价',
    like: '赞同',
    dislike: '不赞同',
  },
  imagePreview: {
    previous: '上一张图片',
    next: '下一张图片',
    expandPanel: '展开预览面板',
    collapsePanel: '收起预览面板',
    showImage: '显示第 {index} 张图片',
  },
  sidebar: {
    home: '社区反馈',
    manual: '用户手册',
    download: '下载客户端',
    myProfile: '个人主页',
    faq: '常见问题',
    createFeedback: '创建反馈',
    loginToCreate: '登录后提交反馈',
    recentSubmitted: '我的反馈',
    followedTopics: '我关注的',
    recentParticipated: '我参与的',
    emptySubmitted: '还没有发布过反馈',
    emptyFollowed: '还没有关注反馈',
    emptyParticipated: '还没有参与过反馈',
    loginToView: '登录后查看',
    loginNow: '去登录',
    syncFailed: '评论已发布，但参与记录同步失败',
    totalComments: '共 {count} 条评论',
    information: '更多信息',
    listSort: '列表排序',
    opensource: '开源代码',
    userAgreement: '用户协议',
    privacyPolicy: '隐私协议',
  },
  exceededRateLimitWarning: '已达到 Gitee 接口的最大请求频率限制，请登录后重试',
  topic: {
    official: '官方',
    author: '作者',
    officialAt: '@空荧酒馆',
    showMore: '显示更多',
    previewTitle: '话题预览',
    backToFeedbackForum: '返回反馈',
    menu: {
      moreActions: '更多操作',
      syncPending: '{action}，同步状态待确认',
      giteeLink: '在Gitee查看',
      toOriginal: '转到原文',
      hideFeedback: {
        text: '隐藏反馈',
        success: '反馈隐藏成功',
        fail: '反馈隐藏失败',
      },
      deleteComment: {
        text: '删除评论',
        title: '确认删除评论',
        confirm: '删除后无法恢复，确定要删除这条评论吗？',
        success: '评论删除成功',
        fail: '评论删除失败',
      },
      closeFeedback: {
        text: '关闭反馈',
        success: '反馈关闭成功',
        fail: '反馈关闭失败',
      },
      copyLink: {
        text: '复制链接',
        success: '已复制',
        fail: '复制失败',
      },
      changeType: {
        text: '更改类型',
        to: '更改为',
        success: '反馈类型修改成功',
        fail: '反馈类型修改失败',
      },
      modifyTags: {
        text: '修改 Tags',
        title: '编辑话题标签（#{id}）',
        success: '反馈标签修改成功',
        fail: '反馈标签修改失败',
      },
      pinTopic: {
        pin: '固定话题',
        unpin: '取消固定',
        success: '反馈固定状态修改成功',
        fail: '反馈固定状态修改失败',
      },
      commentArea: {
        open: '打开评论',
        close: '关闭评论',
        success: '评论区状态修改成功',
        fail: '评论区状态修改失败',
      },
      reopenFeedback: {
        text: '打开反馈',
      },
      cancelTopic: {
        text: '取消话题',
      },
    },
    type: {
      sug: '提建议',
      bug: '提漏洞',
      feat: '我想要',
      ann: '公告',
    },
    state: {
      open: '已开启',
      closed: '已关闭',
      rejected: '已拒绝',
      progressing: '进行中',
    },
    backToTeamBlog: '返回团队博客',
    backToPrevPage: '返回上一页',
  },
  auth: {
    login: '登录',
    logout: '登出',
    loginMsg: '登录 Gitee 账号',
    logoutMsg: '登出 Gitee 账号',
    loginTips: '请登录后重试',
    loginToCheck: '请 %login 后查看 (・ω・)',
    jumpToGiteeTips: '即将跳转到 Gitee 进行授权登录',
    loginSuccess: '登录成功',
    logoutSuccess: '已登出',
    loginFail: '登录失败',
    logoutFail: '登出失败',
    loginAlert: '账号登录',
    loginAlertMsg: '使用 Gitee 账号继续',
    oauthLoginAlert: '前往 Gitee 授权登录',
    oauthLoginAlertMsg: '将离开当前页面，授权完成后会自动返回。',
    account: '账号',
    accountPlaceholder: 'Gitee 登录名或邮箱（不支持手机号或游戏账号）',
    loginHelp: '登录遇到问题？',
    password: '密码',
    passwordPlaceholder: '请输入密码',
    passwordLogin: '登录',
    passwordLoginFail: '登录失败，请检查账号、密码或网络后重试',
    oauthLogin: '使用授权登录',
    or: '或者',
    notGiteeAccountMsg: '没有 Gitee 账号？%signup',
    clickToGiteeSignup: '点击注册 Gitee 账号',
    callback: {
      title: '正在通过 Gitee 授权登录中...',
      description: '请稍等片刻，即将为您重定向到原页面',
      steps: {
        init: '解析授权参数',
        token: '获取访问令牌',
        session: '建立用户会话',
        sso: '同步SSO状态',
        redirect: '准备页面跳转',
      },
      status: {
        pending: '等待中',
        processing: '处理中',
        completed: '已完成',
        failed: '失败',
      },
      error: {
        title: '授权失败',
        retry: '重试',
      },
    },
  },
  aside: {
    contactUs: {
      title: '联系我们',
      desc: '地图功能异常、紧急问题请加入QQ反馈群处理',
      qrcodeLink: 'https://jq.qq.com/?_wv=1027&k=nbveGrfQ',
    },
    teamBlog: {
      text: '团队博客',
      items: [
        {
          text: '【客户端】热更新日志',
          link: '/blog/hotupdatelog-client',
          cover: '/imgs/common/client-updatelog_cover.png',
        },
        {
          text: '【网页版】更新日志',
          link: '/blog/changelog-web',
          cover: '/imgs/common/web-updatelog_cover.png',
        },
        {
          text: '【位置追踪】热更新日志',
          link: '/blog/posts/hotupdatelog-autotrack',
        },
      ],
    },
    suggest: {
      text: '文档推荐',
      items: [],
    },
    recentTopics: {
      text: '最近话题',
      empty: '暂无最近话题',
    },
    info: [
      {
        text: '反馈由 Gitee 提供技术支持',
        link: 'https://gitee.com',
      },
    ],
  },
  comment: {
    comment: '评论',
    commentSuccess: '评论成功：',
    commentFail: '评论失败：',
    commentAfterLogin: '请 %login 后发表评论 (・ω・)',
    commentCount: '评论',
    placeholder: '发表你的评论吧~',
    loadMoreComment: '加载更多评论',
    noComment: '暂无评论',
    noMoreComment: '没有更多评论',
    loadingComment: '评论加载中...',
    reply: '回复',
    commentsClosed: '评论关闭',
  },
  header: {
    view: {
      label: '显示方式',
      card: '卡片',
      compact: '紧凑',
    },
    sort: {
      created: '最近创建',
      updated: '最近更新',
      notesCount: '评论数正序',
    },
    search: {
      placeholder: '搜索反馈',
      allRelatedContentCount: '搜索到相关反馈数',
      resultSummary: '搜索“{query}”，找到 {count} 条相关反馈',
      quickLinks: '快捷链接',
    },
    navigation: {
      allFeedback: '全部反馈',
      bugFeedback: '问题反馈',
      featFeedback: '功能建议',
      closedFeedback: '已结反馈',
      faq: {
        title: '常见问题',
        items: [
          {
            text: 'Alert Dialog',
            link: '/docs/components/alert-dialog',
            desc: 'A modal dialog that interrupts the user with important content and expects a response.',
          },
        ],
      },
    },
  },
  publish: {
    tags: {
      issue: {
        typos: '错别字',
        display: '显示问题',
        login: '登录问题',
        performance: '性能问题',
        translation: '翻译问题',
        other: '其他问题',
        pin: '点位问题',
        docs: '文档问题',
      },
      platforms: {
        all: '全平台可复现',
        web: '网页端问题',
        client: '客户端问题',
      },
    },
    title: '反馈表单',
    type: {
      sug: '提建议',
      bug: '提缺陷',
      feat: '我想要',
      ann: '发公告',
    },
    tagsInput: {
      searchTags: '搜索符合的标签',
      maxTagsLimit: '已达到最大可选标签数量',
      noResultsFound: '没有符合的结果',
      loadFailed: '标签加载失败，请重试',
      retry: '重新加载',
    },
    feedbackForm: {
      submit: '提交反馈',
      keepDraftTitle: '要保留本次编辑吗？',
      keepDraftDescription: '保留后，下次打开反馈表单可以继续编辑。',
      discardDraft: '不保留',
      keepDraft: '保留编辑',
      fieldsNeedAttention: '还有 {count} 个字段需要检查',
      uploadingImages: '正在上传图片 {settled}/{total}',
      publishing: '正在发布反馈…',
      success: '反馈已发布',
      uploadFailed: '图片上传失败，请检查后重试',
      publishFailed: '反馈发布失败，你的内容已保留',
      returnToForm: '返回表单',
      retryImage: '重新上传 {filename}',
      removeImage: '移除 {filename}',
      addImages: '添加图片',
      attachmentsLabel: '反馈图片',
      attachmentsLimit: '已添加 {count}/{max} 张，每张最大 {size}',
      localFilesWarning: '刷新页面后，尚未完成的本地图片不会保留。',
      imageEmpty: '{filename} 是空文件',
      imageInvalidType: '{filename} 的图片格式不受支持',
      imageTooLarge: '{filename} 超过 {size}',
      imageCountExceeded: '最多只能添加 {max} 张图片',
      previewFailed: '无法预览 {filename}',
      formatBold: '切换粗体',
      formatItalic: '切换斜体',
      formatStrike: '切换删除线',
    },
    publishSuccess: '反馈上传成功：',
    publishFail: '反馈上传失败：',
    publishLoading: '反馈提交中',
    form: {
      title: {
        text: '标题',
        placeholder: '请输入标题（5-50个字）',
      },
      type: {
        text: '标签',
        placeholder: '请选择反馈类型（需至少选择一个）',
      },
      content: {
        text: '反馈内容',
        placeholder: '请详细描述你的反馈内容（5-2000字）',
      },
      upload: {
        text: '上传错误截图',
        tip: '请上传相关错误截图或报错信息，最多 %range 张，每张不超过 %size',
        fail: '%filename 上传失败',
        violation: '图片违规！请勿上传任何与反馈无关的图片',
        paste: {
          hint: '可直接粘贴图片',
          formatNotSupported: '剪贴板中的图片格式不受支持',
          sizeExceed: '图片大小 %size MB 超出限制 %maxSize MB',
          formatError: '不支持的图片格式: %format',
        },
      },
    },
  },
  user: {
    menu: {
      giteePage: 'Gitee 个人主页',
      giteeAccountInfo: 'Gitee 账号信息',
    },
    myFeedback: {
      title: '我的反馈',
    },
  },
  readMore: '了解更多',
  translate: {
    translateText: '翻译贴子',
    translatedFrom: '翻译自 {language}',
    settings: '翻译设置',
    settingsDescription: '调整当前设备上的翻译方式',
    autoTranslate: '自动翻译',
    autoTranslateDescription: '自动翻译与当前语言不同的反馈',
    viewOriginal: '查看原文',
    showTranslation: '显示译文',
    conservativeSkip: '内容过短或无法可靠识别语言，未进行翻译',
    error: '翻译失败',
    limit: '已达到最大请求次数，请稍后再试',
    loading: '翻译中...',
  },
  labels: {
    resolved: '已解决',
    submitBug: '提 BUG',
    submitSuggestion: '提建议',
    teamBlog: '团队博客',
    userManual: '使用手册',
    joinCommunity: '加入社区',
    writeThoughts: '写下你的想法...',
    shareExperience: '分享你的经验...',
    askQuestion: '提出你的问题...',
    pasteError: '处理粘贴文件时出错',
    lazyPerson: '这个人很懒，什么都没写~',
    unknown: '未知',
    fetchUserFailed: '拉取用户资料失败',
    myFeedback: '我的反馈',
    submittedFeedback: '提交的反馈',
    personalHomepage: '的个人主页',
    goToProfile: '前往个人资料页',
    imageLoadFailed: '图片加载失败',
    follow: '关注',
    following: '正在关注中',
    unfollow: '取消关注',
    posts: '帖子',
    joinTime: '加入时间',
    privateMessage: '私信',
    commentLabelSuccess: '评论标签成功',
    login: '登录',
    openInEditor: '在你的编辑器打开',
  },
  validation: {
    errors: {
      titleRequired: '标题不能为空',
      contentRequired: '内容不能为空',
      authorRequired: '作者不能为空',
      tooManyTags: '标签过多（最多10个）',
      contentTooShort: '内容至少需要 {min} 个字符',
      contentTooLong: '内容不能超过 {max} 个字符',
      tagsRequired: '至少需要 {min} 个标签',
      tooManyTagsLimit: '标签不能超过 {max} 个',
      tagTooLong: '标签 "{tag}" 超过最大长度 {maxLength} 个字符',
      commentEmpty: '评论内容不能为空',
      invalidImageFormat: '只允许 JPEG、PNG、GIF 和 WebP 图片格式',
      validationError: '验证错误',
      invalidFile: '无效文件',
      fileValidationFailed: '文件验证失败',
      announcementPermission: '只有管理员可以发布公告',
    },
  },
  errors: {
    tooManyRequests: '请求过于频繁，请稍后重试',
    serverError: '服务器错误，请重试',
    notFound: '资源未找到',
    operationFailed: '{operation} 失败：{message}',
    unknownError: '未知错误',
    networkError: '网络连接失败',
    permissionDenied: '权限不足',
    operationFailedRetry: '操作失败，请稍后重试',
    serverNoResponse: '服务器无响应，请稍后重试',
    noResponse: '无响应，请稍后重试',
    networkRequestFailed: '网络请求失败，显示本地数据',
    cannotLoadData: '无法加载数据',
    sortDataLoadFailed: '排序数据加载失败',
    cannotDoToSelf: '不能对自己进行该操作',
    followFailed: '关注失败，请稍后重试',
    unfollowSuccess: '取消关注成功',
    followSuccess: '关注成功',
    permissionInsufficientAnnouncement: '权限不足：只有管理员可以发布公告类型的内容',
    ssoRefreshTokenFailed: 'interknot.site: 刷新token失败，返回数据不完整',
  },
}

export default forum
